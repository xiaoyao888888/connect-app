...
static void print_chrc(struct chrc *chrc, const char *description)
{
	...
	if (!text)
		pr_info("Characteristic  %s %s\n",
					chrc->path, chrc->uuid);
	else
		pr_info(" Characteristic %s %s %s\n",
...
static void read_reply(DBusMessage *message, void *user_data)
{
	...
	if (dbus_set_error_from_message(&error, message) == TRUE) {
		...
		g_rkbt_content->ble_content.cb_ble_recv_fun(NULL,  (char *)error.name, (int32_t *)strlen(error.name), RK_BLE_GATT_CLIENT_READ_BY_LOCAL_ERR);
	...
	//proxy change
	//g_rkbt_content->ble_content.cb_ble_recv_fun(NULL, value, len, RK_BLE_GATT_CLIENT_READ_BY_LOCAL);
...
static void write_reply(DBusMessage *message, void *user_data)
{
	...
	if (dbus_set_error_from_message(&error, message) == TRUE) {
		...
		g_rkbt_content->ble_content.cb_ble_recv_fun(NULL,  (char *)error.name, (int32_t *)strlen(error.name), RK_BLE_GATT_CLIENT_WRITE_RESP_BY_LOCAL_ERR);

		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}

	g_rkbt_content->ble_content.cb_ble_recv_fun(NULL, NULL, 0, RK_BLE_GATT_CLIENT_WRITE_RESP_BY_LOCAL);
...
static void notify_reply(DBusMessage *message, void *user_data)
{
	...
	event = enable ? RK_BLE_GATT_CLIENT_NOTIFY_ENABLE : RK_BLE_GATT_CLIENT_NOTIFY_DISABLE;

	if (dbus_set_error_from_message(&error, message) == TRUE) {
		pr_info("Failed to %s notify: %s\n",
				enable ? "start" : "stop", error.name);

		event = RK_BLE_GATT_SERVER_ERR_NOTIFY_BY_REMOTE;
	...
	if (g_dbus_proxy_get_property(default_attr, "UUID", &iter))
		dbus_message_iter_get_basic(&iter, &uuid);

	if (g_rkbt_content->ble_content.cb_ble_recv_fun)
		g_rkbt_content->ble_content.cb_ble_recv_fun(uuid, NULL, NULL, event);
